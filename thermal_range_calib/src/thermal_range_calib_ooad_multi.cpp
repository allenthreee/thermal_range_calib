...
int main(int argc, char **argv) {
  ...
  // Calibration 类 初始化
  string image_left = "/home/allen/ooad_calib_ws/real_and_ign_data/ign_left.jpg";
  string image_mid = "/home/allen/ooad_calib_ws/real_and_ign_data/ign_mid.jpg";
  string image_right = "/home/allen/ooad_calib_ws/real_and_ign_data/ign_right.jpg";
  string pcd_left = "/home/allen/ooad_calib_ws/real_and_ign_data/ign_left.pcd";
  string pcd_mid = "/home/allen/ooad_calib_ws/real_and_ign_data/ign_mid.pcd";
  string pcd_right = "/home/allen/ooad_calib_ws/real_and_ign_data/ign_right.pcd";

  ImageProcessor left_image_processor(image_left, calib_config_file);
  ImageProcessor mid_image_processor(image_mid, calib_config_file);
  ImageProcessor right_image_processor(image_right, calib_config_file);

  PointCloudProcessor left_point_cloud_processor(pcd_left, calib_config_file);
  PointCloudProcessor mid_point_cloud_processor(pcd_mid, calib_config_file);
  PointCloudProcessor right_point_cloud_processor(pcd_right, calib_config_file);

  Calibration calibra(image_file, pcd_file, calib_config_file);
  calibra.width_ = mid_image_processor.image_width_;
  calibra.height_ = mid_image_processor.image_height_;
  // 这里已经完成了 image edge 和 pointcloud edge 的提取
  ...
  std::vector<pnl_data> pnl_list_left;
  std::vector<pnl_data> pnl_list_mid;
  std::vector<pnl_data> pnl_list_right;
  ...
  for (dis_threshold = 30; dis_threshold > 10; dis_threshold -= 1) {
    ...
    for (int cnt = 0; cnt < 2; cnt++) {
      // left数据有问题，引入误差
      // calibra.buildPnL(calib_params, dis_threshold, true,
      //                   left_image_processor.image_edge_cloud_,
      //                   left_point_cloud_processor.plane_line_cloud_,
      //                   pnl_list_left);
      calibra.buildPnL(calib_params, dis_threshold, true,
                        mid_image_processor.image_edge_cloud_,
                        mid_point_cloud_processor.plane_line_cloud_,
                        pnl_list_mid);
      calibra.buildPnL(calib_params, dis_threshold, true,
                        right_image_processor.image_edge_cloud_,
                        right_point_cloud_processor.plane_line_cloud_,
                        pnl_list_right);

      std::cout << "Iteration:" << iter++ << " Dis:" << dis_threshold
                << " pnl size:" << pnl_list_left.size() << std::endl;
