...
int main(int argc, char **argv) {
  ...
  // Calibration 类 初始化
  ImageProcessor my_image_processor(image_file, calib_config_file);
  PointCloudProcessor my_point_cloud_processor(pcd_file, calib_config_file);

  Calibration calibra(image_file, pcd_file, calib_config_file);
  calibra.width_ = my_image_processor.image_width_;
  calibra.height_ = my_image_processor.image_height_;
  ...
  for (dis_threshold = 30; dis_threshold > 10; dis_threshold -= 1) {
    ...
    for (int cnt = 0; cnt < 2; cnt++) {
      calibra.buildPnL(calib_params, dis_threshold, true,
                        my_image_processor.image_edge_cloud_,
                        my_point_cloud_processor.plane_line_cloud_,
                        pnl_list);

      std::cout << "Iteration:" << iter++ << " Dis:" << dis_threshold
                << " pnl size:" << pnl_list.size() << std::endl;
