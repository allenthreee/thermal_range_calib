...
def visualize_line(direction, point, line_id):
    ...
    start_point = Point()
    start_point.x = point[0] - 2 * direction[0]
    start_point.y = point[1] - 2 * direction[1]
    start_point.z = point[2] - 2 * direction[2]

    end_point = Point()
    end_point.x = point[0] + 2 * direction[0]
    end_point.y = point[1] + 2 * direction[1]
    end_point.z = point[2] + 2 * direction[2]
...
if __name__ == '__main__':
    ...
    plane1 = (np.array([1.5, -1.0, 1.0]), np.array([0.9, -0.1, 0]))  # YZ plane
    plane2 = (np.array([0.5, -1.1, 1.5]), np.array([0.3, 1.0, 0]))  # XZ plane
    plane12 = (np.array([0.5, -1.5, 1.5]), np.array([0.35, 1.0, 0]))  # XZ plane
    plane3 = (np.array([0.5, -0.9, 1.8]), np.array([0.0, 0.0, 0.5]))  # XY plane
    ...
    for planes in [(plane1, plane2), (plane1, plane12), (plane1, plane3), (plane3, plane2), (plane3, plane12)]:
    ...
    for plane, plane_id in [(plane1, 1), (plane12, 12), (plane2, 2), (plane3, 3)]:
