...
pcd_arr = load_point_cloud("/home/allen/calib_ws/data_demo/0lidar.pcd")
print(pcd_arr.shape)
# Load the image
...
def update_params(x):
    ...
    for i in range(len(pcd_arr)):
        ...
        resized_image = cv2.circle(resized_image, (int(points_2d[i][0][0]), int(points_2d[i][0][1])), radius=0, color=(R, G, B), thickness=30)
